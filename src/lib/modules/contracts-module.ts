...
const CONTRACT_SKILLS: SkillSeed[] = [
  ...
  },
  {
    ...
    description: 'Author the organisation\'s own contract templates: get, create, update or archive the reusable agreement bodies that manage_contract renders into real contracts. ONLY these tokens are rendered at contract creation — {{counterparty.name}}, {{counterparty.email}}, {{today}}, {{start_date}}, {{end_date}}, {{value}}, {{currency}}, {{title}}. Any other {{...}} survives into the signed agreement and reads as a defect, so write everything else the customer must fill in as [BRACKETS]. Use when: the business changes its terms, adds a product that needs its own agreement, or asks you to draft a new contract type. NOT for: creating a contract for a specific customer (manage_contract action=create with template_id), or reading the templates that exist (list_contract_templates).',
    category: 'commerce',
    ...
    instructions: 'create returns unrendered_tokens — any {{...}} in the body that will NOT be substituted at contract creation. A non-empty list means the signed agreement would contain literal braces; fix the body and update rather than shipping it.\n\ncreate is idempotent on name: an existing name returns that row with already_existed = true instead of erroring, so use action = update to change a body.\n\nWrite the body as the finished agreement, not as a form: numbered clauses, a signature block, and [BRACKETS] wherever a human must decide something (amounts, notice periods, choice of court).',
  },
  {
