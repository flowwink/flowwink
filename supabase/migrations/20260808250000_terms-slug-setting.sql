-- {{terms_url}} follows the operator's terms page, not a hardcoded slug.
--
-- The /villkor route was a Swedish-only special case; terms now publish
-- through a `terms` BLOCK on any CMS page (the platform's generic public
-- surface), and the agreement's terms link must point wherever the operator
-- actually put that page. general.termsSlug carries the choice; 'villkor'
-- stays the fallback so every existing instance keeps working unchanged.
