...
describe('classify — a defect versus a fact about the instance', () => {
  ...
  it('never lets infrastructure flicker into a baseline', () => {
    // Three skills answered 502 on one run and fine on the next. Baking that in
    // makes the FOLLOWING run report a false recovery and the one after it a
    // false regression — a baseline containing flicker is worse than none.
    expect(classify('Execution failed (502): <html><head><title>502 Bad Gateway')).toBe('transient');
    expect(classify("probe failed: JSON Parse error: Unrecognized token '<'")).toBe('transient');
    expect(classify('fetch failed')).toBe('transient');
    expect(classify('The operation timed out')).toBe('transient');
  });

  it('reads a bare "X required" as a contract gap, not as breakage', () => {
    // Fourth calibration. The pattern matched "is required" and "requires" but
    // not the bare form, so update_purchase_order read as broken.
    expect(classify('Handler exception: purchase_order_id required')).toBe('contract_gap');
    expect(classify('Template not found: (none passed). Run list_contract_templates')).toBe('contract_gap');
    expect(classify('RPC manage_gift_card failed: Gift card <NULL> not found')).toBe('contract_gap');
  });
...
describe('probeFor — never probe anything that sends, deletes or charges', () => {
  ...
  it('reads the enum instead of guessing "list"', () => {
    // The first prober guessed "list" everywhere and reported ten schemas as
    // under-described. Every one of them declared a precise enum — list_stock,
    // list_categories, list_hours — and the prober had ignored it. The
    // instrument was wrong, not the platform.
    expect(probeFor(skill('manage_inventory', {
      properties: { action: { enum: ['list_stock', 'update_stock', 'low_stock'] } },
      required: ['action'],
    }))).toBe('{"action":"list_stock"}');

    expect(probeFor(skill('manage_service_sla', {
      properties: { p_action: { enum: ['set', 'status', 'list_breaches'] } },
      required: ['p_action'],
    }))).toBe('{"p_action":"status"}');
  });

  it('refuses to fire an action whose enum offers nothing read-shaped', () => {
    // Picking an arbitrary enum value could approve, cancel or enrol something.
    expect(probeFor(skill('manage_flowtable_base', {
      properties: { action: { enum: ['create', 'update'] } },
      required: ['action'],
    }))).toBeNull();
  });

  it('skips an action-shaped skill with no enum at all rather than guessing', () => {
    expect(probeFor(skill('mystery', { properties: { action: {} }, required: ['action'] }))).toBeNull();
  });
...
describe('diffAgainstBaseline — regression, not absolute failure', () => {
  ...
  it('ignores a transient in BOTH directions', () => {
    // ok -> transient is not a regression, and transient -> ok is not a
    // recovery. Either would be noise generated by the gateway hiccuping.
    const withTransient = { ...base, gamma: 'transient' } as never;
    const { regressions, recoveries } = diffAgainstBaseline(withTransient, [
      r('alpha', 'transient'), r('gamma', 'ok'),
    ]);
    expect(regressions).toEqual([]);
