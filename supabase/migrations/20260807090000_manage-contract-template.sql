-- An agent could read contract templates but never write one.
--
-- list_contract_templates existed; nothing created or edited a template. So the
-- one artefact a sales organisation iterates on constantly — the wording of its
-- own agreements — was the one thing an operator had to hand back to a human
-- with database access. That is the opposite of the goal.
--
-- rpc: handler, so it reaches every instance through sync:skills without an
-- edge deploy anywhere.
  ...
  ELSIF p_action = 'create' THEN
    ...
    RETURN jsonb_build_object('id', v_row.id, 'name', v_row.name,
      ...
      'unrendered_tokens', (
        ...
        FROM regexp_matches(v_row.body_markdown, '\{\{([^}]+)\}\}', 'g') AS t
...
REVOKE ALL ON FUNCTION public.manage_contract_template(text, text, text, text, text, text, text, text, text, integer, bigint, boolean, boolean) FROM PUBLIC, anon;
